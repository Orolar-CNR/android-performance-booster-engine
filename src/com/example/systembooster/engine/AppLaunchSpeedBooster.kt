...
open class AppLaunchSpeedBooster(
    ...
    override val id = "app_launch_booster"
    override val version = "2.0"
    override val displayName = "App Launch Speed Booster"
    override val capability = EngineCapability.APP_COMPILATION
    override val requiredPrivilege = PrivilegeLevel.UNPRIVILEGED
    override val retryPolicy = RetryPolicy.ON_TIMEOUT
    ...
    override suspend fun prepare() {
        logger.i(TAG, "[$executionId] Preparing app launch speed booster")
    }

    override suspend fun cleanup() {
        logger.i(TAG, "[$executionId] Cleaning up app launch speed booster resources")
    }

    override suspend fun execute(targetPackage: String): EngineResult {
        val startTime = System.currentTimeMillis()
        val optResult = optimizeAppLaunch(targetPackage)
    ...
    open suspend fun optimizeAppLaunch(
        ...
        if (targetPackageName.isBlank()) {
            return@withContext OptimizationResult.Failure.InternalError(
                IllegalArgumentException("targetPackageName must not be blank")
            )
        }
        ...
        try {
            val actualCommand = command ?: listOf(
                "cmd", "package", "compile", "-m", "speed-profile", "-f", targetPackageName
            )

            logger.i(TAG, "[$executionId] Starting compilation optimization for $targetPackageName")
            logger.d(TAG, "[$executionId] Command: ${actualCommand.joinToString(" ")}")
            ...
            if (!completed) {
                logger.e(TAG, "[$executionId] Compilation timeout")
            ...
            logger.i(TAG, "[$executionId] Compilation finished with exit code = $exitCode")
            ...
            logger.e(TAG, "[$executionId] Compilation failed", t)
